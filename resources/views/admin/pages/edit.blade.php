...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl md:text-2xl font-bold">Edit Page: {{ Str::limit($page->name, 30) }}</h1>
            <p class="text-black/45 text-sm mt-1">
                Created {{ $page->created_at->format('M j, Y') }} · Updated {{ $page->updated_at->diffForHumans() }} · {{ $page->word_count }} words
            </p>
        </div>
        <div class="flex items-center gap-4">
            @if ($page->status === 'published')
                <a href="{{ $page->url }}" target="_blank" class="text-xs font-semibold text-black/50 hover:text-ink transition">
                    <i class="fa-solid fa-arrow-up-right-from-square mr-1"></i> View Live
                </a>
            @endif
        ...
        <div class="grid lg:grid-cols-[1fr_340px] gap-6 items-start">
            <div class="bg-white rounded-3xl shadow-card p-5 md:p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $page->name) }}" required class="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $page->slug) }}" required class="flex-1 px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition">
            ...
            <div class="space-y-5">
                <div class="bg-white rounded-3xl shadow-card p-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="relative">
                                <select name="status" id="status" required class="w-full appearance-none px-4 py-2.5 rounded-xl border border-black/10 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary transition pr-10">
                                    ...
                                    <option value="private" @selected(old('status', $page->status) === 'private')>Private</option>
                                </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="sort_order" id="sort_order" min="0" value="{{ old('sort_order', $page->sort_order) }}" class="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition">
                        </div>

                        <div class="flex items-center justify-between gap-4 py-1">
                            <div>
                                <p class="text-sm font-semibold">Set as Homepage</p>
                                <p class="text-black/45 text-xs mt-0.5">
                                    {{ $page->is_homepage ? 'This page is currently the homepage.' : 'Replaces the current homepage.' }}
                                </p>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer shrink-0">
                                <input type="checkbox" name="is_homepage" value="1" class="peer sr-only" @checked(old('is_homepage', $page->is_homepage))>
                        ...
                        <div class="flex items-center justify-between gap-4 py-1">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer shrink-0">
                                <input type="checkbox" name="show_in_menu" value="1" class="peer sr-only" @checked(old('show_in_menu', $page->show_in_menu))>
                                <span class="w-10 h-6 bg-black/10 peer-checked:bg-success rounded-full transition block"></span>
                                <span class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition peer-checked:translate-x-4"></span>
                            </label>
                        </div>
                ...
                <div class="bg-white rounded-3xl shadow-card p-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $page->meta_title) }}" class="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition">
                        </div>
                        <div>
                            <label for="meta_description" class="block text-sm font-medium mb-1.5">Meta Description</label>
                            <textarea name="meta_description" id="meta_description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition resize-none">{{ old('meta_description', $page->meta_description) }}</textarea>
                        </div>
                        <div>
                            <label for="meta_keywords" class="block text-sm font-medium mb-1.5">Meta Keywords</label>
                            <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $page->meta_keywords) }}" placeholder="keyword1, keyword2" class="w-full px-4 py-2.5 rounded-xl border border-black/10 text-sm focus:outline-none focus:ring-2 focus:ring-primary transition">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit" class="flex-1 bg-primary text-ink rounded-full px-4 py-3 text-xs font-semibold hover:bg-primary-dark transition">Update Page</button>
                </div>
            </div>
        </div>
    </form>
