...
@section('content')
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-black/45 text-sm mt-1">{{ $start->format('M j') }} &ndash; {{ $end->format('M j, Y') }} (this month)</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.system.reports.sales.export') }}" class="inline-flex items-center gap-2 bg-white border border-black/10 rounded-full px-4 py-2.5 text-xs font-semibold hover:bg-black/[0.03] transition">
    ...
    <div class="flex flex-wrap items-center gap-1 bg-white rounded-full shadow-card p-1.5 mb-6 overflow-x-auto">
        @foreach ($reportTypes as $key => $label)
            <a href="{{ route('admin.system.reports.index', ['type' => $key]) }}" class="px-5 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition {{ $reportType === $key ? 'bg-primary/20 text-ink' : 'text-black/50 hover:text-ink' }}">{{ $label }}</a>
        @endforeach
    </div>
